...
import numpy as np
from spformer.evaluation.instance_eval_util import Instance
...
class ScanNetEval(object):
    ...
    def evaluate_matches(self, matches):
        ...
        for di, (min_region_sizes, distance_thresh, distance_conf) in enumerate(
                ...
                for li, label_name in enumerate(self.eval_class_labels):
                    ...
                    for m in matches:
                        ...
                        gt_instances = [
                            ...
                            if gt['instance_id'] >= 1000 and gt['vert_count'] >= min_region_sizes
                        ...
                        for (gti, gt) in enumerate(gt_instances):
                            ...
                            if not found_match:
                                hard_false_negative += 1
                        # remove non-matched ground truth instances
                        cur_true = cur_true[cur_match == True]
                        cur_score = cur_score[cur_match == True]

                        # collection non-matched predictions as false positive
                        for pred in pred_instances:
                            ...
                            if not found_gt:
                                ...
                                for gt in pred['matched_gt']:
                                    ...
                                    if gt['vert_count'] < min_region_sizes or gt['med_dist'] > distance_thresh or gt[
                    ...
                    if has_gt and has_pred:
                        ...
                        for idx_res, idx_scores in enumerate(unique_indices):
                            ...
                            # score larger than this position consider as positive
                            fp = num_examples - idx_scores - tp
                            fn = cumsum + hard_false_negative
                            p = float(tp) / (tp + fp)
                            r = float(tp) / (tp + fn)
                            precision[idx_res] = p
                            recall[idx_res] = r

                        # recall is the first point on recall cursive
                        rc_current = recall[0]
    ...
    def compute_average(self, aps, rcs):
        d_inf = 0
    ...
    def evaluate(self, pred_list, gt_list):
        ...
        avgs = self.compute_average(ap_scores, rc_scores)
    ...
    def assign_instances_for_scan(self, preds, gts):
        """get gt instances, only consider the valid class labels even in class agnostic setting"""
